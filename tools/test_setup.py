...
def test_model_files():
    ...
    files_to_check = {
        os.path.join(project_root, 'Model', 'keras_model.h5'): 'Trained model',
        os.path.join(project_root, 'Model', 'labels.txt'): 'Class labels'
    }
    ...
    for filepath, description in files_to_check.items():
        if os.path.exists(filepath):
            ...
            print(f"✅ {description}: {relpath} ({size:,} bytes)")
        else:
            relpath = os.path.relpath(filepath, project_root)
            print(f"⚠️  {description}: {relpath} - NOT FOUND")
            all_exist = False
