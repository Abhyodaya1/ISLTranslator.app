"""
VGCC (VGG16-based) Transfer Learning Model for ISL Hand Sign Recognition
Features:
- VGG16 base model (classic deep CNN)
- Deeper feature extractor for strong baselines
- Optimized for stable accuracy
"""
...
print("=" * 70)
print("[TRAINING] ISL Hand Sign Recognition - VGCC (VGG16) Model")
print("=" * 70)
...
# Save labels
labels_path = os.path.join(MODEL_DIR, "labels_vgcc.txt")
with open(labels_path, "w") as f:
    for idx, label in enumerate(class_names):
        f.write(f"{idx} {label}\n")
print(f"[OK] Labels saved to {labels_path}")

# Build VGCC model (VGG16-based)
print("\n[BUILD] Building VGCC (VGG16) model...")
...
callbacks = [
    ModelCheckpoint(
        filepath=os.path.join(MODEL_DIR, "best_model_vgcc.h5"),
...
# Training
print("\n" + "=" * 70)
print(" Phase 1: Training VGCC with Frozen Base")
print("=" * 70)
...
# Fine-tuning
print("\n" + "=" * 70)
print(" Phase 2: Fine-tuning VGCC")
print("=" * 70)
...
print("[UNFREEZE] Unfroze last 8 layers for fine-tuning")
...
# Save model
model_path = os.path.join(MODEL_DIR, "keras_model_vgcc.h5")
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("VGCC (VGG16) - Loss")
plt.legend()
plt.grid(True)

plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "training_history_vgcc.png")
plt.savefig(plot_path)
print(f"[OK] Training plots saved to {plot_path}")
try:
    plt.savefig(os.path.join(MODEL_DIR, "training_history_vgcc.png"))
except Exception:
    pass

# Final evaluation
print("\n" + "=" * 70)
print(" Final Evaluation - VGCC")
print("=" * 70)
...
print("\n" + "=" * 70)
print("[SUCCESS] VGCC Training Complete!")
print("=" * 70)
